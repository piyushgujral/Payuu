...
(function () {
    ...
    function triggerEngagement(type) {
        ...
        const ref = db.ref('engagementQueue').push();
        const payload = {
            id: ref.key,
            type,
            icon: config.icon,
        ...
        ref.set(payload)
            ...
            .catch(error => {
                ...
                alert('Unable to send engagement overlay. Check Firebase permissions.');
            });
    }
    ...
    function createControls() {
        ...
        box.innerHTML = `
            ...
            <div class="payuu-engagement-buttons">
                <button type="button" class="payuu-engagement-btn like" data-engagement="like">
                    ❤️ LIKE
                </button>
                <button type="button" class="payuu-engagement-btn subscribe" data-engagement="subscribe">
                    🔔 SUBSCRIBE
                </button>
                <button type="button" class="payuu-engagement-btn share" data-engagement="share">
                    📣 SHARE
                </button>
            </div>
        ...
        box.querySelectorAll('[data-engagement]').forEach(button => {
            button.addEventListener('click', () => {
                triggerEngagement(button.dataset.engagement);
            });
        });
    }
    ...
    function addStyles() {
        ...
        style.textContent = `
            .payuu-engagement-controls {
                margin: 0 0 18px;
                padding: 16px;
                border: 1px solid rgba(255, 215, 0, 0.35);
                border-radius: 16px;
                background: linear-gradient(135deg, rgba(255,215,0,.08), rgba(0,240,255,.05));
                box-shadow: 0 0 22px rgba(255,215,0,.08);
            }
            .payuu-engagement-title {
                font-weight: 900;
                letter-spacing: 1px;
                color: #FFD700;
                font-size: .92rem;
            }
            .payuu-engagement-subtitle {
                color: #A0B3C6;
                font-size: .72rem;
                margin: 5px 0 12px;
            }
            .payuu-engagement-buttons {
                display: grid;
                grid-template-columns: repeat(3, 1fr);
                gap: 9px;
            }
            .payuu-engagement-btn {
            ...
            @media (max-width: 600px) {
                .payuu-engagement-buttons { grid-template-columns: 1fr; }
            }
        `;
